...
int main()
{
	...
	conv_layer_t * layer1 = new conv_layer_t( 1, 5, 5, cases[0].data.size );		// 28 * 28 * 1 -> 24 * 24 * 8
...
#pragma pack(pop)
	...
	int pool = (layer3->out.size.x)*(layer3->out.size.y)*(layer3->out.size.z)*(layer3->extend_filter)*(layer3->extend_filter);
	int relu = (layer2->in.size.x)*(layer2->in.size.y)*(layer2->in.size.z);
	int conv = (layer1->out.size.x)*(layer1->out.size.y)*(layer1->out.size.z)*(layer1->extend_filter)*(layer1->extend_filter)*2;
	printf("conv: %d\n", conv/2);
	printf("conv layer time variable: %lf\n", conv/((sum0)/(1.0*100)));
	//printf("conv layer time: %lf\n", (1.0*2*26*26*3*3*1)/((sum0)/(1.0*100)));	
	printf("relu: %d\n", relu);
	printf("relu layer time: %lf\n", (relu*1.0)/((sum1)/(1.0*100)));	
	printf("pool: %d\n", pool);
	printf("pool layer time: %lf\n", (pool*1.0)/((sum2)/(1.0*100)));
